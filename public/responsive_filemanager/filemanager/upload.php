...
try {
    ...
    if ($_SESSION['RF']["verify"] != "RESPONSIVEfilemanager") {
        response(trans_rf('forbidden') . AddErrorLocation(), 403)->send();
        exit;
    }
    ...
    if (!checkRelativePath($fldr)) {
        response(trans_rf('wrong path') . AddErrorLocation())->send();
        exit;
    }
    ...
    $messages = null;
    if (trans_rf("Upload_error_messages") !== "Upload_error_messages") {
        $messages = trans_rf("Upload_error_messages");
    }
    ...
    if (!checkresultingsize($_FILES['files']['size'][0])) {
        ...
        $upload_handler->response['files'][0]->error = sprintf(trans_rf('max_size_reached'), $config['MaxSizeTotal']) . AddErrorLocation();
