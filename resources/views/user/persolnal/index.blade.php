                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="content-left">
                            <x-user.persolnal.side-bar :item="$user" />
                        </div>
                    </div>
                    <div class="col-md-7">
                        <div class="content-right">
                            @if(auth()->guard('user')->id()==$user->id)
                                <div class="post-status">
                                    <form action="{{route('trang-ca-nhan.dang-bai-viet')}}" class="form-post"
                                          method="post" enctype="multipart/form-data">
                                        @csrf
                                        <textarea name="content_post" id="status-content"
                                                  placeholder="Nhập tại đây"></textarea>
                                        <div id="preview_image"></div>
                                        <div class="form-bottom">
                                            @if($errors->count()>0 && $errors->has('content_post'))
                                                <span class="text-danger"
                                                      style="position: absolute;left: 5px">{{$errors->first('content_post')}}</span>
                                            @endif
                                            @if(\Session::has('error_img'))
                                                <span class="text-danger"
                                                      style="position: absolute;left: 5px">{{\Session::get('error_img')}}</span>
                                            @endif

                                            <div class="add-image">
                                                <input type="file" id="input_image" name="image[]" multiple="multiple">
                                                <i class="fas fa-camera"></i>
                                                <span>Thêm ảnh</span>
                                            </div>
                                            <button type="submit" class="submit-post"><i class="fas fa-paper-plane"></i>
                                                Đăng bài
                                            </button>
                                        </div>
                                    </form>
                                    <div class="avatar">
                                        <img src="{{ asset(data_get($user, 'user_detail.expert_avatar')) }}" alt="">
                                    </div>
                                </div>
                            @endif
                            @forelse($post as $i)
                                <div class="posts-item">
                                    <div class="header-posts">
                                        <div class="user-posts">
                                            <x-user.avatar
                                                ...
                                                height="55"
                                                rounded="25"
                                                link="{{ route('trang-ca-nhan.dong-thoi-gian', $i->user->user_code) }}"
                                                avatar="{{ $i->user->getExpertAvatar() }}"
                                            />
                                            <div class="user-name">
                                    ...
                                    <div class="posts-content relative">
                                        <div class="info-desc">
                                        ...
                                        <div class="posts-comment">
                                            @if($auth->check())
                                                <div class="form-comment">
                                                    <div class="form-left">
                                                        <div class="avatar">
                                                            <img
                                                                src={{asset($auth->user()->user_detail->image_url??'')}} alt="">
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="list-comment" id="list-comment{{$i->id}}">
                                                @foreach($i->comment->where('parent_id', null)->where('is_show',1) as $comment)
                                                            ...
                                                            <div class="comment-bottom comment_{{$comment->id}}">
                                                                ...
                                                                <div class="comment-reply">
                                                                    @if($comment->child->count()>0)
                                                                        ...
                                                                        @foreach($comment->child->where('is_show',1) as $child)
                                                                            <div class="comment-item d-flex">
                                                                                <div class="comment-left">
                                                                                    <div class="avatar">
        ...
        $(function () {
                ...
                if (input.files) {
                    ...
                    for (i = 0; i < filesAmount; i++) {
                        ...
                        reader.onload = function (event) {
                            $($.parseHTML('<img style="max-width: 24%;margin-left: 1%;">')).attr('src', event.target.result).appendTo(placeToInsertImagePreview);
                        }
            ...
            $('#input_image').on('change', function (e) {
                if ($(this)[0].files.length > 4) {
                    $('#input_image').val('');
                    $('#preview_image').empty();
                    toastr.error('Chỉ chọn được tối đa 4 ảnh');
                    e.preventDefault();
                } else {
                    $('#preview_image').empty();

                    imagesPreview(this, '#preview_image');
                }
