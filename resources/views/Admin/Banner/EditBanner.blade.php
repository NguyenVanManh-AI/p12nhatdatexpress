...
@section('Content')
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12 text-left">
                    <h4 class="m-0 font-weight-bold text-uppercase">Sửa banner</h4>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form method="post" action="{{route('admin.banner.update', [$item->id, \Crypt::encryptString($item->created_by)])}}">
                        @csrf
                        <div class="row">
                                ...
                                <div class="row">
                                        ...
                                        <div class="form-group select2-custom">
                                            ...
                                            <select class="form-control select2" id="banner_group_id" style="width: 100%;height: 34px !important;" name="banner_group_id" data-placeholder="Nhóm banner" onchange="get_group_by_banner_group(this, '#group_id', '{{route('admin.banner.get_group', '')}}/' + this.value)">
                                                ...
                                                @foreach($banner_group as $bg)
                                                    <option @if(old('banner_group_id') == $bg->id || $item->banner_group_id == $bg->id) {{ 'selected' }} @endif value="{{$bg->id}}" >{{$bg->banner_name}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-lg-6 col-md-12">
                                        <div class="form-group select2-custom">
                                            ...
                                            <select class="form-control select2" id="group_id" style="width: 100%;height: 34px !important;" name="group_id" data-placeholder="Chuyên mục">
                                                ...
                                                @foreach($group as $g)
                                                    <option @if(old('group_id') == $g->id || $item->group_id == $g->id) {{ 'selected' }} @endif value="{{$g->id}}" >{{$g->group_name}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-lg-6 col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="link" class="form-control" id="link" value="{{ (old('link')) ?? $item->link}}">
                                            @if($errors->has('link'))
                                                <small class="text-danger error-message-custom">
                                                    {{$errors->first('link')}}
                                                </small>
                                            ...
                                            <div class="mt-3">
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio" name="target_type" id="target_type_1" value="0" {{old('target_type') == 0 || $item->target_type == 0 ? 'checked' : ''}}>
                                                    <label class="form-check-label" for="target_type_1">Tab hiện hành</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio" name="target_type" id="target_type_2" value="1" {{old('target_type') == 1 || $item->target_type == 1 ? 'checked' : ''}}>
                                    ...
                                    <div class="col-lg-6 col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="banner_title" class="form-control" id="banner_title" value="{{ (old('banner_title')) ?? $item->banner_title}}">
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control float-right timedates" name="date" placeholder="Chọn ngày bắt đầu - Ngày kết thúc"
                                               value="@if(old('date')){{old('date')}} @elseif($item->date_from) {{date('d/m/Y h:i A', $item->date_from)}} - {{date('d/m/Y h:i A', $item->date_to)}}@endif">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-12 col-sm-12 col-md-6 col-lg-6 p-2">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="banner_code" id="html-code" rows="5" style="height: 250px !important;">{{old('banner_code') ?? $item->banner_code}}</textarea>
                                </div>
    ...
    <script>
        jQuery(document).ready(function ($) {
            $(function () {
                ...
                // Load Image
                previewOneImgFromInputText('#image_url', '#add-image-default-url', '', '{{$item->image_url}}')
                // Reset form
                ...
                // Load group
                get_group_by_banner_group('#banner_group_id', '#group_id', '{{route('admin.banner.get_group', '')}}/' + {{old('banner_group_id') ?? $item->banner_group_id ?? null}},
                    {{old('group_id') ?? $item->group_id ?? null}});
            });
        });
    </script>
    <script>
