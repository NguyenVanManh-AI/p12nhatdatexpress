...
@section('Title', 'Danh sách bài viết khuyến mãi | Mã khuyến mãi')

@section('Content')
  <x-admin.breadcrumb active-label="Danh sách bài viết khuyến mãi" :parents="[
    [
        'label' => 'Admin',
        'route' => 'admin.thongke',
    ],
  ...
  <section class="content">
    <div class="container-fluid">
      <x-admin.table-filter search-place-holder="Nhập mã, tiêu đề..." add-route="{{ route('admin.promotion-news.create') }}"
      date-range-filter />
      ...
      <div class="row">
        <div class="col-12">
                    ...
                    <td>
                      ...
                      <div>
                        <div class="flex-start">
                          <span class="mr-2 icon-small-size text-muted">
                    ...
                    <td>
                      <div>
                        <x-admin.action-button
                          ...
                          :item="$item"
                          url="{{ route('admin.promotion-news.edit', $item->id) }}"
                        />
                        <x-admin.action-button
                          action="delete"
                          :check-role="$check_role"
                          :item="$item"
                          url="{{ route('admin.promotion-news.delete-multiple', ['ids' => $item->id]) }}"
                        />
                      </div>
                    </td>
                  </tr>
          ...
          <x-admin.table-footer :check-role="$check_role" :lists="$news" :count-trash="$countTrash"
            view-trash-url="{{ route('admin.promotion-news.trash') }}"
            delete-url="{{ route('admin.promotion-news.delete-multiple') }}" />
        </div>
      </div>
    </div>
  </section>
