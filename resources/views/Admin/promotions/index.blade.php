...
@section('Content')
  <x-admin.breadcrumb active-label="Quản lý mã khuyến mãi" :parents="[
      [
          'label' => 'Admin',
          'route' => 'admin.thongke',
      ],
  ...
  <section class="content">
    <div class="container-fluid">
      <x-admin.table-filter search-place-holder="Nhập mã..." add-route="{{ route('admin.promotion.create') }}"
        date-range-filter />
      ...
      <div class="row">
        <div class="col-12">
                ...
                @foreach ($promotions as $item)
                  <tr>
                    ...
                    <td class="text-left">
                      ...
                      </span><span>{{ \Carbon\Carbon::createFromTimestamp($item->created_at)->format('d/m/Y h:i') }}</span><br>
                      <span class="font-weight-bold">Ngày bắt đầu:
                      </span><span>{{ \Carbon\Carbon::createFromTimestamp($item->date_from)->format('d/m/Y h:i') }}</span><br>
                      <span class="font-weight-bold">Ngày kết thúc:
                      </span><span>{{ \Carbon\Carbon::createFromTimestamp($item->date_to)->format('d/m/Y h:i') }}</span>
                    </td>
                    <td>
                    ...
                    <td class="text-left">
                      <div class="flex-column">
                        <x-admin.action-button
                          ...
                          :item="$item"
                          url="{{ route('admin.promotion.edit', $item->id) }}"
                        />
                        <x-admin.action-button
                          action="delete"
                          :check-role="$check_role"
                          :item="$item"
                          url="{{ route('admin.promotion.delete-multiple', ['ids' => $item->id]) }}"
                        />
                      </div>
                    </td>
                  </tr>
          ...
          <x-admin.table-footer
            ...
            view-trash-url="{{ route('admin.promotion.trash') }}"
            delete-url="{{ route('admin.promotion.delete-multiple') }}" />
        </div>
      </div>
    </div>
  </section>
