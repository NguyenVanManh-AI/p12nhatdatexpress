...
@section('Title', 'Danh sách bình luận dự án | Quản lý bình luận')
@section('Style')
  ...
  <div class="row m-0 px-2 pt-3">
    <ol class="breadcrumb mt-1">
      ...
      <li class="recye px-2 pt-1 ml-1">
        <a href="{{route('admin.comment.trash-comment-project')}}">
          Thùng rác
        </a>
      </li>
      @endif
...
<div class="container-fluid px-3 mt-2">
  <form method="get" action="{{route("admin.comment.list-project")}}">
    <div class="row m-0">
  ...
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-12 text-center">
        <h1 class="m-0 font-weight-bold">QUẢN LÝ BÌNH LUẬN DỰ ÁN</h1>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
</div>
    ...
    <div class="row">
      <div class="col-12">
        <div class="table-responsive">
          <form action="{{route('admin.comment.trashlist-comment-project')}}" id="formtrash" method="post">
            @csrf
            <table class="table table-bordered text-center table-hover table-custom " id="table" style="min-width: 1050px">
              <thead>
                <tr>
                  ...
                  <th scope="col" width="10%">Liên kết</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="title_role text-wrap text-left" style="word-break: break-word;">
                    <p class="mb-0" style="line-height: 1.5em;height: 3em;overflow: hidden;">{{$item->comment_content}}</p>
                  </td>
                ...
                <td >
                  ...
                  <a href="{{ route('home.project.project-detail',$item->project_url) }}" target="_blank" rel="noopener noreferrer">{{$item->project_url}}</a>
                </td>
                <td class="text-left">
                  <div class="d-flex flex-column">
                    <div class="ml-2 mb-2">
                      <i class="fa fa-eye mr-2"></i>
                      <a href="{{ route('home.project.project-detail', $item->project_url) }}" class="text-primary" target="_blank" rel="noopener noreferrer">
                        Xem
                      </a>
                    </div>
            ...
            <div class="btn-group ml-1">
              ...
              <div class="dropdown-menu ">
               ...
               <a class="dropdown-item list_unlocked" type="button" href="javascript:{}">
                <i class="fa fa-undo bg-primary p-1 mr-2 rounded"
                style="color: white !important;font-size: 16px;width: 23px"></i>Bỏ chặn tài khoản
                <input type="hidden" name="action" value="trash">
              </a>
               @endif
               @if($check_role == 1 || key_exists(5, $check_role))
      ...
      <div class="d-flex flex-row align-items-center view-trash" style="margin-top: -5px">
        ...
        <div class="link-custom" >
          <a href="{{route('admin.comment.trash-comment-project')}}"><span  style="color: #347ab6">Xem thùng rác</span>
...
<script>
  $('.locked').click(function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = "/admin/comment/project/locked-account/" + id + "/" + created_by;
      }
    });
  });
   //Bỏ Chặn tài khoản
   $('.unblock_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.comment.locked-account-comment-project', '')}}/" + id;
                }
            });
        });   
  ...
  $('.forbidden').click(function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = "/admin/comment/project/forbidden-account/" + id + "/" + created_by;
      }
    });
  });
  ...
  $('.delete').click(function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = "/admin/comment/project/delete/" + id + "/" + created_by;
      }
    });
  });
  ...
  $('.forbidden-list').click(function () {
    $('#formtrash').attr('action', "/admin/comment/project/forbidden-comment")
    var id = $(this).data('id');
    Swal.fire({
  ...
  $('.un-forbidden-list').click(function () {
    $('#formtrash').attr('action', "/admin/comment/project/un-forbidden-comment")
    var id = $(this).data('id');
    Swal.fire({
  ...
  $('.locked-list').click(function () {
    $('#formtrash').attr('action', "/admin/comment/project/locked-comment")
    var id = $(this).data('id');
    Swal.fire({
   ...
   // bỏ Chặn nhiều tài khoản
   $('.list_unlocked').click(function () {
    $('#formtrash').attr('action', "/admin/comment/project/un-locked-comment")
    var id = $(this).data('id');
    Swal.fire({
      title: 'Chặn nhiều tài khoản',
