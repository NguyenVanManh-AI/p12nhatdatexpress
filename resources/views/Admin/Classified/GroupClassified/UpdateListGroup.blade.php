...
@section('Content')
    <section>
        ...
        <h4 class="mb-2 text-bold text-center">SỬA DANH MỤC TIN RAO</h4>
        <form action="{{route('admin.groupclassified.edit',[$item->id,Crypt::encryptString($item->created_by)])}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row m-0 px-3 pb-3">
                        ...
                        <div class="col-12 col-sm-12 col-md-12 col-lg-12 p-2">
                            ...
                            <div class="search-reponsive">
                                <select class="form-control select2" style="width: 100%;height: 34px !important;"
                                        name="parent_id">
                                    <option {{($item->parent_id ==2)?"selected":""}} value="2">Danh mục nhà đất bán</option>
                                    @foreach($group_ndb as $item1)
                                        <option {{($item->parent_id == $item1->id)?"selected":""}} value="{{$item1->id}}">---- {{$item1->group_name}}</option>
                                    @endforeach
                                    <option {{($item->parent_id ==10)?"selected":""}} value="10">Danh mục nhà đất cho thuê</option>
                                    @foreach($group_ndct as $item1)
                                        <option {{($item->parent_id == $item1->id)?"selected":""}} value="{{$item1->id}}">---- {{$item1->group_name}}</option>
                                    @endforeach
                                    <option {{($item->parent_id ==18)?"selected":""}} value="18">Danh mục cần mua - cần thuê</option>
                                    @foreach($group_cmct as $item1)
                                        <option {{($item->parent_id == $item1->id)?"selected":""}} value="{{$item1->id}}">---- {{$item1->group_name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div style="width: 40%">
                                ...
                                <div class="input-group">
                                    <div style="position: relative;width: 100%">
                                        ...
                                        <input class="form-control pl-5" id="image_url" value="{{$item->image_url}}" name="image_url" onchange="previewOneImgFromInputText(this,'#add-image')"
                ...
                <div class="col-12 vol-sm-12 col-md-12 col-lg-12 p-2">
                    ...
                    <textarea class="js-admin-tiny-textarea" name="group_description" id="group_description" style="width: 100%;height: 300px">{{old('group_url')??$item->group_description}}</textarea>
                </div>
...
@section('Script')
    <script type="text/javascript">
        $(document).ready(function(){
                previewOneImgFromInputText('#image_url','#add-image')
        });
    </script>
    <script>
        $(function () {
