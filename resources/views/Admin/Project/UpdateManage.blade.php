...
<form method="get" action="{{route('admin.project.update-manage')}}">
  <div class="row m-0 p-3">
    <div class="col-12 col-sm-12 col-md-5 col-lg-5 p-0">
      <div  class="search-reponsive pr-5">
        <input class="form-control required" type="" name="name_project" placeholder="Nhập tên dự án" value="{{ app('request')->input('name_project') }}">
      </div>
    </div>
    ...
    <div class="col-12 col-xs-12 col-sm-12 col-md-4 col-lg-4 p-3 mb-4 cusor-point" >
      <div>
        <a href="{{route('admin.project.update-manage')}}">
          <img src="{{ asset("system/images/icons/update1.png")}}" width="100%" height="100%" style="border: 5px solid #007bff;border-radius: 50%">
        </a>
    ...
    <div class="col-12 col-xs-12 col-sm-12 col-md-4 col-lg-4 p-3 mb-4 cusor-point" >
      <div>
        <a href="{{route('admin.project.update-price-rent')}}">
          <img src="{{ asset("system/images/icons/update2.png")}}" width="100%" height="100%" style="border: 5px solid #d7d7d7;border-radius: 50%">
        </a>
          ...
          <table class="table table-bordered text-center table-custom" id="table">
            <thead>
              <tr>
                ...
                <th scope="col"  style="width: 13%">Tình trạng</th>
                <th scope="col" style="width: 13%">Tình trạng mới</th>
            ...
            <tbody>
              @forelse ($listUpdate as $key => $item)
                <tr>
                    <td>{{($listUpdate->currentPage() - 1) * $listUpdate->perPage() + $loop->index + 1}}</td>
                    <td>
                      <a target="__blank" class="name-text" href="{{route('home.project.project-detail', $item['project_url'])}}">{{$item["project_name"]}}</a>
                    </td>
                    <td>{{$item["progress_update_old"]}}</td>
                    <td>{{$item["progress_update"]}}</td>
                    <td>
                        @if($item['change_date'])
                            {{date('d/m/Y', $item['change_date'])}}
                        @else
                            ---
                        @endif
                    </td>
...
<script type="text/javascript">
  ...
  $('.confirm').click(function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = "/admin/project/update-manage/confirm-update-manage/" + id + "/" + index;
      }
    });
  });
  $('.recover').click(function () {
    ...
    Swal.fire({
      title: 'Hoàn tác',
      text: "Khi hoàn tác, Tình trạng sẽ được đưa về trạng thái lúc chưa cập nhật",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#3085d6',
      ...
      if (result.isConfirmed) {
        window.location.href = "/admin/project/update-manage/recover-update-manage/" + id + "/" + index;
      }
    });
  });
</script>
