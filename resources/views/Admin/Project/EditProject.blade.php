...
@section('Title', 'Sửa dự án | Quản lý dự án')
@section('Style')
...
@section('Content')
    ...
    {{--                        @include('Admin.Project.ViewProject');--}}
    {{--                    </div>--}}
    {{--                </div>--}}
    {{--            </div>--}}
    {{--        </div>--}}
    ...
    <div class="bg-white box-main-reponsive box-main">
        <form id="form" action="{{route('admin.project.update', [$project->id, \Crypt::encryptString($project->created_by)])}}" method="post" enctype="multipart/form-data" novalidate>
            @csrf
            <div class="row">
                    ...
                    <div>
                        ...
                        <select class="form-control select2" style="width: 100%;height: 34px !important;" name="group_id" id="group_id"
                            ...
                            @foreach($category as $item)
                                <option value="{{$item->id}}" {{ old('group_id') == $item->id || $project->group_id == $item->id ? 'selected="selected"' : "" }}>{{$item->group_name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div id="locationRequest">
                        ...
                        <select class="form-control select2 " name="province_id" id="province" data-placeholder="Tỉnh / Thành phố"
                            ...
                            @foreach($province as $item)
                                <option value="{{$item->id}}" {{old('province_id') == $item->id || $project->location->province_id == $item->id ? "selected" : ""}}>{{$item->province_name}}</option>
                            @endforeach
                        </select>
                        ...
                        @endif
                        <div class="space-line"></div>
                        <input class="form-control required" type="text" name="address" id="road" placeholder="Địa chỉ đường" value="{{ old('address') ?? $project->location->address}}" />
                ...
                <div class="map-box map-box-reponsive col-sm-12 col-md-6 " style="height: inherit">
                    ...
                    <input type="hidden" name="map_latitude" id="map_latitude" value="{{ old('map_latitude') ?? $project->location->map_latitude}}">
                    <input type="hidden" name="map_longtitude" id="map_longtitude" value="{{ old('map_longtitude') ?? $project->location->map_longtitude }}">
                    ...
                    <div class="box-dash">
                        ...
                        <div class="row pt-2 pr-3">
                            <div class="col-12 col-sm-12 col-md-4 col-lg-3 box_input ">
                                ...
                                <div>
                                    <input class="form-control required select" type="number" name="project_price" value="{{ old('project_price') ?? $project->project_price_new ?? $project->project_price}}"
                                           maxlength="6" oninput="maxLengthInput(this)" />
                                    <select  class="pl-2 float-right" id="giatien"  name="price_unit_id">
                                        @foreach($unit_sell as $item)
                                            <option value="{{$item->id}}" {{old('price_unit_id') == $item->id || $project->price_unit_id == $item->id ? "selected" : ""}}>{{$item->unit_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-12 col-sm-12 col-md-4 col-lg-3 box_input mtdow10">
                                ...
                                <div>
                                    <input class="form-control required select" type="number" name="project_rent_price" value="{{ old('project_rent_price') ?? $project->project_rent_price_new ?? $project->project_rent_price }}"
                                           maxlength="6" oninput="maxLengthInput(this)" />
                                    <select id="giatien" class="pl-2"  name="project_unit_rent_id">
                                        @foreach($unit_rent as $item)
                                            <option value="{{$item->id}}" {{old('project_unit_rent_id') == $item->id || $project->project_unit_rent_id == $item->id ? "selected" : ""}}>{{$item->unit_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-12 col-sm-12 col-md-4 col-lg-3 box_input mtdow10">
                                ...
                                <div>
                                    <input class="dientich form-control" placeholder="Từ" name="project_area_from" type="number" value="{{ old('project_area_from') ?? $project->project_area_from}}"
                                           maxlength="6" oninput="maxLengthInput(this)"/>
                                    <div class=" space-area text-center font-weight-bold mx-1">
                                        -
                                    </div>
                                    <input class="dientich form-control" placeholder="Đến" name="project_area_to" type="number" value="{{ old('project_area_to') ?? $project->project_area_to}}"
                                           maxlength="6" oninput="maxLengthInput(this)" />
                                </div>
                            ...
                            <div class="col-12 col-sm-12 col-md-4 col-lg-3 box_input mtdow10">
                                ...
                                <input class="form-control required" placeholder="" name="project_scale" type="number" value="{{ old('project_scale') ?? $project->project_scale}}"
                                       maxlength="6" oninput="maxLengthInput(this)" />
                                ...
                                <div class="form-group">
                                    <select class="form-control select2" style="width: 100%;height: 34px !important;" name="project_juridical">
                                        @foreach($legal as $item)
                                            <option value="{{$item->id}}" {{old('project_juridical') == $item->id || $project->project_juridical == $item->id ? "selected" : "" }}>{{$item->param_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class=" mt15 col-12 col-sm-12 col-md-4 col-lg-3 select-box mtdow10">
                                ...
                                <select class="form-control select2" style="width: 100%;height: 34px !important;" name="project_direction">
                                    @foreach($direction as $item)
                                        <option value="{{$item->id}}" {{ old('project_direction') == $item->id || $project->project_direction == $item->id ? "seleted" : ""}}>{{$item->direction_name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12 col-sm-12 col-md-4 col-lg-3 box_input mt15">
                                ...
                                <input class="form-control required" name="project_road" type="text" value="{{ $project->project_road ?? old('project_road')}}">
                            ...
                            <div class="col-12 col-sm-12 col-md-4 col-lg-3 select-box">
                                ...
                                <div class="d-flex align-items-center">
                                    <label class="search-reponsive form-control-409 mr-2">
                                        <input type="checkbox" value="1" class="checkbox-forced-colors-checked" id="bank_sponsor" {{old('bank_sponsor') == 1 || $project->bank_sponsor == 1 ? "checked" : ""}} name="bank_sponsor">
                                    </label>
            ...
            <div class="row mt-5">
                <div class="col-12 bg-white">
                    <div class="box-dash info">
                        ...
                        @php
                            $list_utility = json_decode($project->list_utility);
                        @endphp
                                ...
                                <div class="col-6 col-sm-6 col-md-3 col-lg-3 box_checkbox">
                                    <div class="support-vay2">
                                        <label class="form-control-409 mr-2">
                                            <input type="checkbox" class="checkbox-forced-colors-checked" name="list_utility[{{$item->id}}]" id="utility-{{$item->id}}"
                                                   value="{{$item->id}}" {{  old("list_utility.". $item->id) || (is_array($list_utility) && in_array( $item->id, $list_utility)) ? 'checked' : ''}} />
                                        </label>
            ...
            <div class="row mt-5">
                ...
                {{--                                <div class="choose-image" data-toggle="modal" data-target="#modalFileThumb" style="cursor: pointer">--}}
                {{--                                    <img src="{{ asset("system/images/icons/upload-file.png")}}" class="mt-4">--}}
                {{--                                    --}}{{--                                <span class="desc d-block">Kéo & Thả ảnh tại đây!</span>--}}
                {{--                                    <p>Bạn chỉ có thể chọn 1 hình làm ảnh làm thumbnail <br> cho dự án.</p>--}}
                {{--                                    <span class="btn btn-upload">Tải ảnh lên</span>--}}
                {{--                                    --}}{{--                                <input type="file" id="thumb" name="image_thumbnail" accept="image/*" />--}}
                {{--                                    <input type="hidden" value="{{old('image_thumbnail')}}" id="image_url_file_thumb" name="image_thumbnail" onchange="previewOneImgFromInputText(this, '#add-thumbnail')">--}}
                {{--                                </div>--}}
                {{--                            </div>--}}
                {{--                            <small class="text-danger error-message-custom" id="msg_image_thumbnail"></small>--}}
                {{--                            @if($errors->has('image_thumbnail'))--}}
                {{--                                <small class="text-danger error-message-custom">--}}
                {{--                                    {{$errors->first('image_thumbnail')}}--}}
                {{--                                </small>--}}
                {{--                            @endif--}}
                {{--                        </div>--}}
                {{--                        <div class="col-12 col-sm-12 col-md-6 col-lg-6 p-2">--}}
                {{--                            <label>&nbsp;</label>--}}
                {{--                            <br />--}}
                {{--                            <div class="form-row add-gallery justify-content-center" id="add-thumbnail" style="border:1px solid #ccc">--}}
                {{--                            </div>--}}
                {{--                        </div>--}}
                {{--                    </div>--}}
                {{--                    <div class="row">--}}
                ...
                {{--                            <br>--}}
                {{--                            <div style="height: 230px;background: white;border: 1px solid #b7b7b7;padding:8px">--}}
                {{--                                <div class="choose-image" data-toggle="modal" data-target="#modalFile" style="cursor: pointer">--}}
                {{--                                    <img src="{{ asset("system/images/icons/upload-file.png")}}" class="mt-4">--}}
                {{--                                    --}}{{--                                <span class="desc d-block">Kéo & Thả ảnh tại đây!</span>--}}
                ...
                {{--                            <label>&nbsp;</label>--}}
                {{--                            <br />--}}
                {{--                            <ul class="form-row add-gallery p-0 py-1" id="add-gallery" style="border:1px solid #ccc">--}}
                {{--                            </ul>--}}
                {{--                        </div>--}}
                {{--                    </div>--}}
                {{--                </div>--}}
                ...
                <div class="col-12 col-sm-12 col-md-4 col-lg-4 p-2">
                    {{--                    <label>Video</label>--}}
                    {{--                    <br />--}}
                    <input class="form-control required mb-2" placeholder="Nhập đường dẫn video youtube" name="video" id="video" onblur="setIframeYoutube(this, '#iframeYoutube')" value="{{old('video') ?? $project->video}}">
                    <small class="text-danger error-message-custom" id="msg_video"></small>
                    @if($errors->has('video'))
                        <small class="text-danger error-message-custom">
                            {{$errors->first('video')}}
            ...
            <div class="row px-2">
               ...
               <textarea class="js-admin-tiny-textarea" id="project_content" name="project_content" style="width: 100%;height: 300px">
                {{old('project_content') ?? $project->project_content}}
              </textarea>
                    <input type="hidden" id="num_word" name="num_word" value="{{old('num_word') ?? $project->num_word}}"/>
                </div>
                <div class="vitri-editer tiny-box hide">
                 <textarea class="js-admin-tiny-textarea" name="location_descritpion" id="location_descritpion" style="width: 100%;height: 300px">
                  {{old('location_descritpion') ?? $project->location_descritpion }}
                </textarea>
                </div>
                <div class="tienich-editer tiny-box hide">
                 <textarea class="js-admin-tiny-textarea" name="utility_description" id="utility_description" style="width: 100%;height: 300px">
                  {{old('utility_description') ?? $project->utility_description}}
                </textarea>
                </div>
                <div class="matbang-editer tiny-box hide">
                 <textarea class="js-admin-tiny-textarea" name="ground_description" id="ground_description" style="width: 100%;height: 300px">
                  {{old('ground_description') ?? $project->ground_description}}
                </textarea>
                </div>
                <div class="phaply-editer tiny-box hide">
                 <textarea class="js-admin-tiny-textarea" name="legal_description" id="legal_description" style="width: 100%;height: 300px">
                  {{old('legal_description') ?? $project->legal_description}}
                </textarea>
                </div>
                <div class="thanhtoan-editer tiny-box hide">
                 <textarea class="thanhtoan-editer js-admin-tiny-textarea" id="payment_description" name="payment_description"
                   ...
                   {{old('payment_description') ?? $project->payment_description}}
                 </textarea>
                </div>
            ...
            <div class="px-2">
                ...
                <div class="box-seo-main-reponsive row mt-4 mb-5">
                    <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pr-2">
                        ...
                        <input class="form-control required" id="project_url" name="project_url" value="{{old('project_url') ?? $project->project_url}}" />
                    ...
                    <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pl-2">
                        ...
                        <input class="form-control required" id="meta_title" name="meta_title" value="{{old('meta_title') ?? $project->meta_title}}" />
                    ...
                    <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pr-2 tach-input"
                        ...
                        <textarea class="w-100 p-2" name="meta_key" id="meta_keyword" rows="5" style="height: auto;">{{old('meta_key') ?? $project->meta_key}}</textarea>
                    ...
                    <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pl-2">
                        ...
                        <textarea class="w-100 p-2" name="meta_desc" rows="5" id="meta_desc">{{old('meta_desc') ?? $project->meta_desc}}</textarea>
                ...
                <div class="row mt-4 mb-4 row text-center d-flex justify-content-center">
                    <button id="btnPreview" class="btn btn-primary mr-2" style="width: 120px;border-radius: 0 !important" type="button">
                        Xem trước
                    </button>
                    <div class=" p-0 pl-2">
                        <button class="btn btn-success" style="width: 120px;border-radius: 0 !important">Hoàn tất</button>
                    </div>
                </div>
            </div>
        </form>
...
@section('Script')
    ...
    <script type="text/javascript">
        let isClicked = false;
        function callback(){
            let initLocation = {lat: {{$project->location->map_latitude}}, lng: {{$project->location->map_longtitude}}}
        ...
        $(document).ready(function () {
            ...
            get_district('#province_id', '{{route('param.get_district')}}', '#district', {{old('province_id') ?? $project->location->province_id}}, {{old('district_id') ?? $project->location->district_id}})
            get_ward('#district_id', '{{route('param.get_ward')}}', '#ward', {{old('district_id') ?? $project->location->district_id}} , {{old('ward_id') ?? $project->location->ward_id}})
            get_furniture('#group_id', '{{route('param.get_furniture')}}', '#furniture', {{old('group_id') ?? $project->group_id}} , {{old('project_furniture') ?? $project->project_furniture ?? "null"}});
            get_progress('#group_id', '{{route('param.get_progress')}}', '#progress', {{old('group_id') ?? $project->group_id}}, {{old('project_progress') ?? $project->project_progress ?? "null"}})
            setIframeYoutube('#video', '#iframeYoutube')
            @if($project->image_thumbnail)
            previewOneImgFromInputText('#image_url_file_thumb', '#add-thumbnail', '', '{{$project->image_thumbnail}}')
            @endif
            @if($project->image_url)
            previewImgFromInputText('#image_url_file', '#add-gallery', '#image_url_order', '', '{{$project->image_url}}')
            @endif
            ...
            async function getPreviewProject(url, form){
                ...
                data.push({name: 'id', 'value': {{$project->id}} });
                jQuery.ajax({
                    url: url,
                    type: "POST",
                    // dataType: "json",
