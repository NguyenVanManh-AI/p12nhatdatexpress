...
@section('Title', 'Chỉnh sửa nhóm quản trị | Tài khoản quản trị')
@section('Style')
@endsection
@section('Content')
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12 text-left">
                    <h4 class="m-0 font-weight-bold text-uppercase">Cập nhật nhóm quản trị</h4>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form method="post" action="{{route('admin.manage.group.update', [$role->id, \Crypt::encryptString($role->created_by)])}}" id="form_data">
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                                    <label for="title">Tiêu đề</label>
                                    <input type="text" name="title" class="form-control" id="title" value="{{ (old('title')) ?? $role->role_name}}">
                                    @if($errors->has('title'))
                                                                ...
                                                                <div style="border-bottom:1px dotted #ccc; padding:2px 0;">
                                                                    <label class="checkbox-inline title" for="page[{{$page_child->id}}][{{$page_per->id}}][check]">
                                                                        <input class="checkbox firstCheckbox" type="checkbox" value="1" name="page[{{$page_child->id}}][{{$page_per->id}}][check]" id="page[{{$page_child->id}}][{{$page_per->id}}][check]"
                                                                            {{old("page.". $page_child->id. ".". $page_per->id.".check") ||
                                                                            (isset($role->role_content[$page_child->id][$page_per->id]['check']) && $role->role_content[$page_child->id][$page_per->id]['check']) ? 'checked' : ''}}
                                                                        >
                                                                    ...
                                                                    @if($page_per->permission_type)
                                                                        ...
                                                                        <label class="checkbox-inline">
                                                                            <input class="checkbox" type="checkbox" value="1" name="page[{{$page_child->id}}][{{$page_per->id}}][self]" id="page[{{$page_child->id}}][{{$page_per->id}}][self]"
                                                                                   {{old("page.". $page_child->id. ".". $page_per->id.".self") ||
                                                                                    (isset($role->role_content[$page_child->id][$page_per->id]['self']) && $role->role_content[$page_child->id][$page_per->id]['self']) ? 'checked' : ''}}>
                                                                        ...
                                                                        <label class="checkbox-inline">
                                                                            <input class="checkbox" type="checkbox" value="1" name="page[{{$page_child->id}}][{{$page_per->id}}][group]" id="page[{{$page_child->id}}][{{$page_per->id}}][group]"
                                                                                   {{old("page.". $page_child->id. ".". $page_per->id.".group") ||
                                                                                 (isset($role->role_content[$page_child->id][$page_per->id]['group']) && $role->role_content[$page_child->id][$page_per->id]['group']) ? 'checked' : ''}}>
                                                                        ...
                                                                        <label class="checkbox-inline">
                                                                            <input class="checkbox allCheckbox" type="checkbox" value="1" name="page[{{$page_child->id}}][{{$page_per->id}}][all]" id="page[{{$page_child->id}}][{{$page_per->id}}][all]"
                                                                                   {{old("page.". $page_child->id. ".". $page_per->id.".all") ||
                                                                                    (isset($role->role_content[$page_child->id][$page_per->id]['all']) && $role->role_content[$page_child->id][$page_per->id]['all']) ? 'checked' : ''}}>
                                                                ...
                                                                <div style="border-bottom:1px dotted #ccc; padding:2px 0;">
                                                                    <label class="checkbox-inline title" for="page[{{$page->id}}][{{$page_per->id}}][check]">
                                                                        <input class="checkbox firstCheckbox" type="checkbox" value="1" name="page[{{$page->id}}][{{$page_per->id}}][check]" id="page[{{$page->id}}][{{$page_per->id}}][check]"
                                                                            {{old("page.". $page->id. ".". $page_per->id.".check") ||
                                                                            (isset($role->role_content[$page->id][$page_per->id]['check']) && $role->role_content[$page->id][$page_per->id]['check']) ? 'checked' : ''}}>
                                                                    ...
                                                                    @if($page_per->permission_type)
                                                                        ...
                                                                        <label class="checkbox-inline">
                                                                            <input class="checkbox" type="checkbox" value="1" name="page[{{$page->id}}][{{$page_per->id}}][self]" id="page[{{$page->id}}][{{$page_per->id}}][self]"
                                                                                   {{old("page.". $page->id. ".". $page_per->id.".self") ||
                                                                                (isset($role->role_content[$page->id][$page_per->id]['self']) && $role->role_content[$page->id][$page_per->id]['self']) ? 'checked' : ''}}>
                                                                        ...
                                                                        <label class="checkbox-inline">
                                                                            <input class="checkbox" type="checkbox" value="1" name="page[{{$page->id}}][{{$page_per->id}}][group]" id="page[{{$page->id}}][{{$page_per->id}}][group]"
                                                                                   {{old("page.". $page->id. ".". $page_per->id.".group") ||
                                                                                (isset($role->role_content[$page->id][$page_per->id]['group']) && $role->role_content[$page->id][$page_per->id]['group']) ? 'checked' : ''}}>
                                                                        ...
                                                                        <label class="checkbox-inline">
                                                                            <input class="checkbox allCheckbox" type="checkbox" value="1" name="page[{{$page->id}}][{{$page_per->id}}][all]" id="page[{{$page->id}}][{{$page_per->id}}][all]"
                                                                                   {{old("page.". $page->id. ".". $page_per->id.".all") ||
                                                                                    (isset($role->role_content[$page->id][$page_per->id]['all']) && $role->role_content[$page->id][$page_per->id]['all']) ? 'checked' : ''}}>
            ...
            $(function () {
                ...
                $('input[type="checkbox"]').change(function (){
                    if ($(this).hasClass('checkbox firstCheckbox')){
                        // $(this).parent().siblings().find('input:checkbox').map((k, v) => $(v).prop('disabled', !$(this).is(":checked")))
