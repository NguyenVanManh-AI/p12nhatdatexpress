...
@section('Title', 'Danh sách tin khuyến mãi | Mã khuyến mãi')
@section('Style')
...
<div class="row m-0 px-3 pt-3">
  ...
  <ol class="breadcrumb mt-1">
    ...
    <li class="add px-2 pt-1 ml-1 check">
      <a href="{{route('admin.promotion.add-news-promotion')}}">
...
<h4 class="text-center font-weight-bold mb-3 mt-2">DANH SÁCH TIN KHUYẾN MÃI</h4>
<!-- Main content -->
    ...
    <div class="row">
      <div class="col-12">
        <div class="table-responsive">
          <form action="{{route('admin.promotion.news-trashlist')}}" id="formtrash" method="post">
            @csrf
            <table class="table table-bordered text-center table-custom" id="table" style="min-width: 1050px">
              ...
              <tbody>
                @foreach ($list_news_promotion as $item )
                <tr>
                  <td class="active">
               ...
               <td>
                <div>
                  ...
                  <div class="float-left ml-2">
                    ...
                    <a href="{{route('admin.promotion.edit_news-promotion',[$item->id,\Crypt::encryptString($item->created_by)])}}" class="text-primary ">Chỉnh sửa</a>
                  </div>
                  <br>
                  @endif
                  @if($check_role == 1  ||key_exists(5, $check_role))
  ...
  <div class="float-left mb-2 w160">
    <div class="manipulation d-flex mr-4 pt-1">
     ...
     <div class="view-trash">
       <a href="{{route('admin.promotion.news-trash')}}"><i class="far fa-trash-alt"></i> Xem thùng rác</a>
...
<div class="float-right mb-2 w370">
 <div class="d-flex align-items-center">
  <div class="count-item" >Tổng cộng: @empty($list_news_promotion) {{0}} @else {{$list_news_promotion->total()}} @endempty items</div>
  <div class="count-item-reponsive" style="display: none">@empty($list_news_promotion) {{0}} @else {{$list_news_promotion->total()}} @endempty items</div>
  <div class="float-right">
   @if($list_news_promotion)
   {{ $list_news_promotion->render('Admin.Layouts.Pagination') }}
   @endif
 </div>
</div>
</div>
  ...
  $('.delete').click(function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = "/admin/promotion/news-delete/" + id + "/" + created_by;
      }
    });
  });
