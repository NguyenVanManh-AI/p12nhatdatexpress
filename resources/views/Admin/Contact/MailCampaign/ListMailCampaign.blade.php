...
@section('Title', 'Danh sách chiến dịch | Quản lý liên hệ')
@section('Style')
        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12 text-center">
                    <h1 class="m-0 font-weight-bold">DANH SÁCH CHIẾN DỊCH EMAIL KHÁCH HÀNG</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="table-responsive">
                        <form action="{{route('admin.email-campaign.delete-mail-campaign-list')}}" id="formtrash"
                              method="post">
                            @csrf
...
{{--                                                        <a href="{{route('admin.mail-campaign.list-send-mail',[$item->id,\Crypt::encryptString($item->created_by)])}}">Xem--}}
{{--                                                            chi tiết</a>--}}
{{--                                                    </div>--}}
{{--                                                @endif--}}
                                                ...
                                                @if($item->is_action == 0 && $item->total_receipt_mail < 1)
                                                    @if($check_role == 1  ||key_exists(2, $check_role))
                                                        <div class="float-left ml10 mb-2">
                                                            ...
                                                            <a href="{{route('admin.contact.campaign.edit',[$item->id,\Crypt::encryptString($item->created_by)])}}">Chỉnh
                                                                sửa</a>
                                                        </div>
                                                    @endif
                                                @endif
                    ...
                    <div class="table-bottom d-flex align-items-center justify-content-between  pb-5">
                            ...
                            @if($check_role == 1 || key_exists(8, $check_role))
                                <div class="d-flex flex-row align-items-center view-trash" style="margin-top: -5px">
                                    ...
                                    <div class="link-custom">
                                        <a href="{{route('admin.contact.campaign.trash')}}"><span
                                                style="color: #347ab6">Xem thùng rác</span>
                                            <span class="badge badge-pill badge-danger trashnum"
                                                  style="font-weight: 500;">{{$trash_num}}</span>
                                        </a>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="d-flex align-items-center">
                            {{-- tổng cộng mẫu mail --}}
                            <div class="count-item">Tổng cộng: @empty($list) {{0}} @else {{$list->total()}} @endempty
                                items
                            </div>
                            @if($list)
                                {{-- phân trang --}}
