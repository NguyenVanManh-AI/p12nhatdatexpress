...
@section('Title', 'Sửa sự kiện | Sự kiện')
@section('Style')
            ...
            <div class="row" id="create-event" style="display: block; position: relative; width: 100%; transform: unset; top: 0; left: 0">
               <div class="col-12 d-flex justify-content-center w-100">
                ...
                <div class="col-12 wrapper">
                    <form method="post" class="form form-add form-popup" action="{{route('admin.event.update', [$item->id, \Crypt::encryptString($item->created_by)])}}">
                        @csrf
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for=""> Tên sự kiện</label>
                                <input type="text" class="form-control" name="event_title" onblur="changeToSlug(this, '#event_url')" id="event_title" placeholder="Tên sự kiện" value="{{old('event_title') ?? $item->event_title}}">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="date" class="form-control" placeholder="Ngày diễn ra" name="start_date" value="{{ old('start_date', date('Y-m-d', $item->start_date)) }}">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" placeholder="Địa điểm tổ chức" name="take_place" value="{{old('take_place') ?? $item->take_place}}">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" placeholder="Thời gian diễn ra sự kiện"  name="start_time" value="{{ old('start_time') ?? date('H:i', $item->start_date)}}">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="on-off-toggle">
                                    <input class="on-off-toggle__input" type="checkbox" id="project_highlight" name="is_highlight" value="1" {{ old('is_highlight', $item->is_highlight) ? 'checked' : '' }} />
                                    <label for="project_highlight" class="on-off-toggle__slider"></label>
                                </div>
                            </div>

{{--                            <textarea rows="7" placeholder="Nội dung sự kiện" id="event_content" name="event_content">{{ old('event_content') ??  $item->event_content}}</textarea>--}}
                                <div class="col-12">
                                    <label for="">Nội dung sự kiện</label>
                                    <textarea id="event_content" name="event_content" class="js-admin-tiny-textarea" placeholder="Nội dung sự kiện">{{old('event_content') ??  $item->event_content}}</textarea>
...
{{--                                            <div class="button button-select">Chọn ảnh có sẵn</div>--}}
                                            <input type="text" id="image_header_url" name="image_header_url" value="{{old('image_header_url') ?? $item->image_header_url}}" onchange="changeImageHeader(this)">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="wrap-out">
                                    <div class="wrap-in" data-target="#modalInvationImage" data-toggle="modal">
                                        ...
                                        <div class="buttons">
                                            <div class="button button-upload" style="padding: 5px; pointer-events: none">Tải ảnh lên</div>
{{--                                            <div class="button button-select">Chọn ảnh có sẵn</div>--}}
                                            <input type="text" id="image_invitation_url" name="image_invitation_url" value="{{old('image_invitation_url') ?? $item->image_invitation_url}}" onchange="changeImageInvitation(this)">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="row" id="locationRequest">
                                <div class="form-group col-md-4">
                                    <input type="text" class="form-control" placeholder="Địa chỉ" id="address" name="address" value="{{old('address') ?? $item->location->address}}">
                                ...
                                <div class="form-group col-md-4">
                                    <select name="province_id" id="province" class="form-control"
                                        ...
                                        @foreach($province as $p)
                                            <option @if(request()->query('province_id') == $p->id || $item->location->province_id == $p->id) {{ 'selected' }} @endif value="{{$p->id}}">{{$p->province_name}}</option>
                                        @endforeach
                                    </select>
...
{{--                        </div>--}}
                        <input type="submit" value="Sửa sự kiện" class="mb-4"/>
{{--                        <p class="last">* Sự kiện có thể được  kiểm duyệt lên tới 48 giờ trước khi hiển thị trên website</p>--}}
                        <div class="box-seo-main-reponsive row mt-4 mb-5 d-none">
                            <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pr-2">
                                ...
                                <input class="form-control required" id="event_url" name="event_url" value="{{old('event_url') ?? $item->event_url}}" />
                            ...
                            <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pl-2">
                                ...
                                <input class="form-control required" id="meta_title" name="meta_title" value="{{old('meta_title') ?? $item->meta_title}}" />
                            ...
                            <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pr-2 tach-input"
                                ...
                                <textarea class="w-100 p-2" name="meta_key" id="meta_keyword" rows="5" style="height: auto;">{{old('meta_key') ?? $item->meta_key}}</textarea>
                            ...
                            <div class="box-seo-reponsive col-12 col-sm-12 col-md-6 col-lg-6 box_input p-0 pl-2">
                                ...
                                <textarea class="w-100 p-2" name="meta_desc" rows="5" id="meta_desc">{{old('meta_desc') ?? $item->meta_desc}}</textarea>
...
@section('Script')
    ...
    <script>
        let isClicked = false;
        function callback(){
            let initLocation = {lat: {{$item->location->map_latitude}}, lng: {{$item->location->map_longtitude}}}
        ...
        $(function () {
            ...
            @if(old('image_header_url') ?? $item->image_header_url)
                    $('.image_header_url').prop('src', '{{asset(old('image_header_url') ?? $item ->image_header_url)}}')
                    $(".image_header_url").parents(".upload-image").css({ opacity: "1", height: "auto" });
                @endif

                @if(old('image_invitation_url') ?? $item->image_invitation_url)
                    $('.image_invitation_url').prop('src', '{{asset(old('image_invitation_url') ??  $item ->image_invitation_url)}}')
                    $(".image_invitation_url").parents(".upload-image").css({ opacity: "1", height: "auto" });
                @endif
                ...
                @if((old('province_id') || $item->location->province_id) && (old('district_id') || $item->location->district_id))
                    get_district('#province', '{{route('param.get_district')}}', '#district', {{old('province_id') ?? $item->location->province_id}}, {{old('district_id') ?? $item->location->district_id}})
                @endif
