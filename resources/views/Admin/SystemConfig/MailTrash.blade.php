...
{{--            @endif--}}
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12">
                    <h4 class="m-0 text-bold text-center">QUẢN LÝ THÙNG RÁC</h4>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered text-center table-custom" id="table">
                            ...
                            @foreach($mailtemplate as $item )
                                <tr>
                                    <td class=" active">
                                        <input type="checkbox"  class="select-item checkbox order{{$item->id}}" value="{{$item->id}}" name="select_item[]" />
                                    </td>
                                    <td class="input_rounded">
                                            <input type="number" disabled class="show_order" name="order{{$item->id}}" value="{{$item->show_order}}" >
                                    ...
                                    <td>
                                        <div class="d-flex flex-row justify-content-around align-content-center table_action">
                                            <div><i class="fas fa-undo-alt"></i><a onclick="untrash({{$item->id}},'{{\Crypt::encryptString($item->created_by)}}')" class=" stretched-link text-primary  ml-1">Khôi phục</a></div>
                                            <div><i class="fas fa-times mr-2"></i><a onclick="delete_item({{$item->id}},'{{\Crypt::encryptString($item->created_by)}}')" class="text-danger action_delete stretched-link">Xóa</a></div>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                                ...
                                <div class="btn-group ml-1">
                                    ...
                                    <div class="dropdown-menu">
                                        @if($check_role == 1  ||key_exists(6, $check_role))
                                            <a class="dropdown-item untrash_list"  href="javascript:{}">
...
{{--                                            </a>--}}
                            ...
                            <div class="display d-flex align-items-center mr-4">
                                <span>Hiển thị:</span>
                                <form method="get" action="{{route('admin.system.mail.mail_trash')}}">
                            ...
                            <a href="{{route('admin.templates.index')}}" class="btn btn-primary">Quay lại</a>
                            </div>
                        </div>
...
@section('Script')
    ...
    <script>
        ...
        function untrash(id,created_by)
        {
            Swal.fire({
                title: 'Khôi phục mẫu mail',
                text: "Sau khi khôi phục sẽ được chuyển ra khỏi thùng rác",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href= "/admin/system-config/untrash-mail/"+id+"/"+created_by;

                }
            })
        }
        function delete_item(id,created_by)
        {
            Swal.fire({
                title: 'Xóa mẫu mail',
                ...
                if (result.isConfirmed) {
                    window.location.href= "/admin/system-config/delete/"+id+"/"+created_by;
                }
            })
        }
        $('.untrash_list').click(function (){
            // id = $(this).data('id');
            Swal.fire({
                title: 'Khôi phục mẫu mail',
