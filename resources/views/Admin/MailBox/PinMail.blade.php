                  ...
                  @foreach ($list as $item )
                    <div class="item item{{$item->id}}">
                        ...
                        <div class="color-star {{$item->mailbox_pin  == 1 ? "active":""}} position{{$item->id}}" data-id="{{$item->id}}">
                          <span class=""><i data-mailbox_pin="{{$item->mailbox_pin}}" class="fas fa-star"></i></span>
                        </div>
                    <div class="content">
        ...
        <div class="table-bottom d-flex align-items-center justify-content-between mb-4  pb-5">
            <div class="text-left d-flex align-items-center">
                ...
                <div class="display d-flex align-items-center mr-4">
                    <span>Hiển thị:</span>
                    <form method="get" action="{{route('admin.mailbox.pin')}}">
            ...
            <div class="d-flex align-items-center" >
                <div class="count-item">Tổng cộng:  <span class="count">{{$list->total()}}</span>
                    items
                </div>
                @if($list)
