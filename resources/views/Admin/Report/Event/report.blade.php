                ...
                <div class="col-12">
                    <div class="table-contents">
                        <table class="table">
                            ...
                            <tbody>
                            <form action="{{route('admin.report.event.list_action')}}" id="list_action" method="post">
                                @csrf
                                ...
                                @forelse($list as $item)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="text-left mt-2">Đơn vị tổ chức: {{data_get($item->user_detail, 'fullname')}}</div>
                                        </td>
                                        <td>
                                            <div class="content-report">
                                                <div class="list-report mb-2">
                                                    @foreach($item->report_event->groupBy('report_type') as $i)
                                                        <div class="item mb-2">{{$i[0]->report_group->content}}<span class="text-gray">({{count($i)}})</span></div>
                                                    @endforeach
                                                </div>
                                                @if(count($item->report_event->groupBy('report_type')) > 3)
                                                    <span class="view-more">Hiện thêm <span>»</span></span>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            {{date('d/m/Y',$item->report_event->first()->report_time)}}
                                        </td>
                                        <td>
    ...
    <script type="text/javascript">
        ...
        $('.block_display').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.event.block', '')}}/" + id;
                }
            });
        });
        // Khôi phục hiển thị
        $('.unblock_display').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.event.unblock', '')}}/" + id;
                }
            });
        });
        // Chặn tài khoản
        ...
        $('.create-noti').click(async function (){
            ...
            const { value: formValues } = await Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: "{{route('admin.report.event.create_noti')}}",
                        type:"POST",
                        data:{
                            id : id,
                            mail_title:title,
