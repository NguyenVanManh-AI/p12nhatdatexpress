...
@section('Title', 'Báo cáo | Bình luận tin rao')
@section('Style')
                ...
                <div class="col-12">
                    <div class="table-contents">
                        <table class="table">
                            ...
                            <tbody>
                            <form action="{{route('admin.report.classified.comment.list_action')}}" id="list_action" method="post">
                                @csrf
                                        ...
                                        <td>
                                            <div class="content-report">
                                                <div class="list-report mb-2">
                                                    @foreach($item->report_comment_classified->groupBy('report_type') as $i)
                                                        <div class="item mb-2">{{$i[0]->report_group->content}}<span class="text-gray">({{count($i)}})</span></div>
                                                    @endforeach
                                                </div>
                                                @if(count($item->report_comment_classified->groupBy('report_type')) >3)
                                                    <span class="view-more">Hiện thêm <span>»</span></span>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            {{date('d/m/Y',$item->report_comment_classified->first()->report_time)}}
                                        </td>
                                        <td>
...
@section('Script')
    ...
    <script type="text/javascript">
        ...
        $('.report_false').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.report-false', '')}}/" + id;
                }
            });
        });
        // chặn hiển thị
        $('.block_display').click(function () {
            ...
            var created_by = $(this).data('created_by');
            Swal.fire({
                title: 'Chặn hiển thị bình luận',
                text: "Sau khi xác nhận sẽ chặn hiển thị bình luận",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.block', '')}}/" + id;
                }
            });
        });
        // Khôi phục hiển thị
        $('.unblock_display').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.unblock', '')}}/" + id;
                }
            });
        });
        // Chặn tài khoản
        $('.block_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.block-account', '')}}/" + id;
                }
            });
        });
        //Bỏ Chặn tài khoản
        $('.unblock_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.unblock-account', '')}}/" + id;
                }
            });
        });
        // Cấm tài khoản
        $('.forbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.forbidden', '')}}/" + id;
                }
            });
        });
        ...
        $('.unforbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.unforbidden', '')}}/" + id;
                }
            });
        });
        // Xóa tài khoản
        $('.delete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.delete', '')}}/" + id;
                }
            });
        });
        // Khôi phục tài khoản
        $('.undelete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.classified.comment.restore', '')}}/" + id;
                }
            });
        });
        ...
        $('.create-noti').click(async function (){
            ...
            const { value: formValues } = await Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: "{{route('admin.report.classified.comment.create_noti')}}",
                        type:"POST",
                        data:{
                            id : id,
                            mail_title:title,
