...
@section('Title', 'Báo cáo | Tài khoản')
@section('Style')
                ...
                <div class="col-12">
                    <div class="table-contents">
                        <table class="table">
                            ...
                            <tbody>
                            <form action="{{route('admin.report.account.action_list')}}" id="list_action" method="post">
                                @csrf
                                ...
                                @forelse($list as $item)
                                    <tr>
                                        ...
                                        <td>{{$item->id}}</td>
                                        <td>
                                            <div class="text-center title"><a href="javascript:{}">{{$item->username}}</a></div>
                                        </td>
                                        <td>
                                            <div class="content-report">
                                                <div class="list-report mb-2">
                                                    @foreach($item->report_account->groupBy('report_type') as $i)
                                                        <div class="item mb-2">{{$i[0]->report_group->content}}<span class="text-gray">({{count($i)}})</span></div>
                                                    @endforeach
                                                </div>
                                                @if(count($item->report_account->groupBy('report_type')) >3)
                                                    <span class="view-more">Hiện thêm <span>»</span></span>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            {{date('d/m/Y',$item->report_account->first()->report_time)}}
                                        </td>
                                        <td>
                                           @if($item->is_deleted == 1)
                                               <span class="text-red">Xóa tài khoản</span>
                                           @elseif($item->is_forbidden)
                                                <span class="text-red">Cấm tài khoản</span>
                                            @elseif($item->is_locked ==1||$item->lock_time>=time())
                                                <span class="text-red">Chặn 7 ngày</span>
                                            @else
                                        ...
                                        <td>
                                            ...
                                            <div style="display: flex;flex-direction: column;text-align: start">
                                                ...
                                                @if($item->is_locked ==0||$item->lock_time<time())
                                                <div class="ml-2 mb-1">
                                               ...
                                               @else
                                                ...
                                                <div class="ml-2 mb-1">
                                                    ...
                                                    <a href="#" class="" data-toggle="modal" data-target="#modalCreateNotify"
                                                       data-user-id="{{$item->id}}" data-user-username="{{$item->username}}">
                                                        Tạo thông báo
                                ...
                                <div class="btn-group ml-1">
                                    ...
                                    <div class="dropdown-menu">
                                        @if($check_role == 1  ||key_exists(2, $check_role))
                                            ...
                                            <a class="dropdown-item list_block" type="button" href="javascript:{}">
                                                <i class="fas fa-lock bg-red p-1 mr-2 rounded"
                                                   style="color: white !important;font-size: 15px"></i>Chặn tài khoản
                                            </a>
                                            <a class="dropdown-item list_unblock" type="button" href="javascript:{}">
                                            ...
                                            <a class="dropdown-item list_delete" type="button" href="javascript:{}">
                                                <i class="fas fa-times bg-red p-1 mr-2 rounded"
                                                   style="color: white !important;font-size: 15px"></i>Xóa tài khoản
                                            </a>
                                            <a class="dropdown-item list_undelete" type="button" href="javascript:{}">
                                                <i class="fas fa-undo-alt bg-primary p-1 mr-2 rounded"
...
@section('Script')
    ...
    <script type="text/javascript">
        ...
        $('.report_false').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.report-false', '')}}/" + id;
                }
            });
        });
        // Chặn tài khoản
        $('.block_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.block', '')}}/" + id;
                }
            });
        });
        //Bỏ Chặn tài khoản
        $('.unblock_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.unblock', '')}}/" + id;
                }
            });
        });
        // Cấm tài khoản
        $('.forbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.forbidden', '')}}/" + id;
                }
            });
        });
        ...
        $('.unforbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.unforbidden', '')}}/" + id;
                }
            });
        });
        // Xóa tài khoản
        $('.delete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.trash', '')}}/" + id;
                }
            });
        });
        // Khôi phục tài khoản
        $('.undelete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.untrash', '')}}/" + id;
                }
            });
        });
        $('.list_report_false').click(function () {
            Swal.fire({
                title: 'Báo cáo sai',
                text: "Sau khi đồng ý sẽ thay đổi trạng thái",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
    ...
    $('.list_unblock').click(function () {
        ...
        $('.list_unforbidden').click(function () {
            Swal.fire({
                title: 'Bỏ cấm tài khoản',
                text: "Sau khi đồng ý sẽ hủy cấm tài khoản",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
        ...
        $('.create-noti').click(async function (){
            ...
            const { value: formValues } = await Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: "{{route('admin.report.account.create_noti')}}",
                        type:"POST",
                        data:{
                            id : id,
                            mail_title:title,
