...
@section('Title', 'Báo cáo | Bình luận bài viết')
@section('Style')
                ...
                <div class="col-12">
                    <div class="table-contents">
                        <table class="table">
                            ...
                            <tbody>
                            <form action="{{route('admin.report.account.comment.list_action')}}" id="list_action" method="post">
                                @csrf
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="text-left text-gray">
                                                <p class="mb-0">Tài khoản: {{$item->user->username}}</p>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="content-report">
                                                <div class="list-report mb-2">
                                                    @foreach($item->report_post_comment->groupBy('report_type') as $i)
                                                        <div class="item mb-2">{{$i[0]->report_group->content}}<span class="text-gray">({{count($i)}})</span></div>
                                                    @endforeach
                                                </div>
                                                @if(count($item->report_post_comment->groupBy('report_type')) >3)
                                                    <span class="view-more">Hiện thêm <span>»</span></span>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            {{date('d/m/Y',$item->report_post_comment->first()->report_time)}}
                                        </td>
                                        <td>
    ...
    <script type="text/javascript">
        ...
        $('.report_false').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.report-false', '')}}/" + id;
                }
            });
        })
        // chặn hiển thị
        $('.block_display').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.block', '')}}/" + id;
                }
            });
        });
        // Khôi phục hiển thị
        $('.unblock_display').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.unblock', '')}}/" + id;
                }
            });
        });
        // Chặn tài khoản
        $('.block_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.block-account', '')}}/" + id;
                }
            });
        });
        //Bỏ Chặn tài khoản
        $('.unblock_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.unblock-account', '')}}/" + id;
                }
            });
        });
        // Cấm tài khoản
        $('.forbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.forbidden', '')}}/" + id;
                }
            });
        });
        ...
        $('.unforbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.unforbidden', '')}}/" + id;
                }
            });
        });
        // Xóa tài khoản
        $('.delete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.delete', '')}}/" + id;
                }
            });
        });
        // Khôi phục tài khoản
        $('.undelete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.comment.restore', '')}}/" + id;
                }
            });
        });
        ...
        $('.create-noti').click(async function (){
            ...
            const { value: formValues } = await Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: "{{route('admin.report.account.comment.create_noti')}}",
                        type:"POST",
                        data:{
                            id : id,
                            mail_title:title,
