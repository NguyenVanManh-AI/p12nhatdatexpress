                ...
                <div class="col-12">
                    <div class="table-contents">
                        <table class="table">
                            ...
                            <tbody>
                            <form action="{{route('admin.report.account.post.list_action')}}" id="list_action" method="post">
                                @csrf
                                ...
                                @forelse($list as $item)
                                    <tr>
                                        ...
                                        <td>{{$item->id}}</td>
                                        <td>
                                            <div class="text-left text-bold"><p href="javascript:{}">{{$item->post_content}}</p></div>
                                        ...
                                        <td>
                                            <div class="content-report">
                                                <div class="list-report mb-2">
                                                    @foreach($item->report_post->groupBy('report_type') as $i)
                                                        <div class="item mb-2">{{$i[0]->report_group->content}}<span class="text-gray">({{count($i)}})</span></div>
                                                    @endforeach
                                                </div>
                                                @if(count($item->report_post->groupBy('report_type')) >3)
                                                    <span class="view-more">Hiện thêm <span>»</span></span>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            {{date('d/m/Y',$item->report_post->first()->report_time)}}
                                        </td>
                                        <td>
...
@section('Script')
    ...
    <script type="text/javascript">
        ...
        $('.report_false').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.report-false', '')}}/" + id;
                }
            });
        })
        // chặn hiển thị
        $('.block_display').click(function () {
            ...
            var created_by = $(this).data('created_by');
            Swal.fire({
                title: 'Chặn hiển thị bài viết',
                text: "Sau khi xác nhận sẽ chặn hiển thị bài viết",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.block', '')}}/" + id;
                }
            });
        });
        // Khôi phục hiển thị
        $('.unblock_display').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.unblock', '')}}/" + id;
                }
            });
        });
        // Chặn tài khoản
        $('.block_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.block-account', '')}}/" + id;
                }
            });
        });
        //Bỏ Chặn tài khoản
        $('.unblock_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.unblock-account', '')}}/" + id;
                }
            });
        });
        // Cấm tài khoản
        $('.forbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.forbidden', '')}}/" + id;
                }
            });
        });
        ...
        $('.unforbidden').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.unforbidden', '')}}/" + id;
                }
            });
        });
        // Xóa tài khoản
        $('.delete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.delete', '')}}/" + id;
                }
            });
        });
        // Khôi phục tài khoản
        $('.undelete_account').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{route('admin.report.account.post.restore', '')}}/" + id;
                }
            });
        });
        ...
        $('.create-noti').click(async function (){
            ...
            const { value: formValues } = await Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    $.ajax({
                        url: "{{route('admin.report.account.post.create_noti')}}",
                        type:"POST",
                        data:{
                            id : id,
                            mail_title:title,
