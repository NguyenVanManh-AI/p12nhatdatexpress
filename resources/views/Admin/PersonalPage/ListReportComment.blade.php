...
@section('Title', 'Báo cáo bình luận | Trang cá nhân')
@section('Style')
        ...
        <div class="row m-0 pt-2 ">
          <div class="col-12 col-sm-12 col-md-5 col-lg-5 box_input px-0">
           <div class="input-reponsive-search pr-3">
             <input class="form-control required" type="" name="user_post_name" placeholder="Nhập từ khóa" value="{{ app('request')->input('user_post_name') }}">
           </div>
         </div>
            ...
            <div class="search-reponsive input-reponsive-search col-12 col-sm-12 col-md-4 col-lg-4 pr-0">
              <div class="search-reponsive ">
                <select name="report_content" class="form-control select2" style="width: 100%;height: 34px !important;">
                  ...
                  <option value="{{$item->content}}" @if(request('report_content') == $item->content) selected @endif>{{$item->content}}</option>
                  @endforeach
                </select>
              </div>
            </div>
      ...
      <div class="col-12">
        <div class="table-responsive" >
          <form action="{{-- {{route('admin.comment.trashlist-comment-user-post')}} --}}" id="formtrash" method="post">
            @csrf
            <table class="table table-bordered text-center table-hover table-custom" id="table" style="min-width: 1000px !important">
              <thead>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class=" pt-4">
                  <span class="font-weight-bold event-title">{{$ListComment->username}}</span><br>
                  <span>
                    @if($ListComment->is_forbidden ==1)
                    (Đã cấm)
                    @endif
                    @if($ListComment->is_locked == 1 && $ListComment->lock_time > time())
                    (Đã chặn 7 ngày)
                    @endif
                  </span>
                  <br>
                ...
                <td>
                  <div class="report-reasion toggle-show-more{{$ListComment->id}}" style="font-size: 14px;">
                    ...
                    @if($ListReport->user_post_comment_id == $ListComment->id && $ListReport->report_position == 2)
                ...
                <td class="py-2 px-0">
                  <div>
                    ...
                    @if($check_role == 1  ||key_exists(2, $check_role))
                      @if($ListComment->confirm_status == 0)
                          <div class="float-left ml-2 mb-1">
                              <i class="icon-setup fa fa-history mr-2 " ></i>
                              <a class="text-primary wrong" href="{{route('admin.personal-page.comment.wrong-report', $ListComment->id)}}" style="cursor:pointer">Báo cáo sai</a>
                          </div>
                          <br>
                      @endif
                      ...
                      @if($ListComment->is_show == 1)
                          <div class="float-left ml-2 cusor-point mb-1">
                              ...
                              <a  class="text-danger block-display" href="{{route('admin.personal-page.comment.hide-show', $ListComment->id)}}">Chặn hiển thị</a>
                          </div>
                          <br>
                      @else
                          <div class="float-left ml-2 cusor-point mb-1">
                              <i class="icon-setup fa fa-undo mr-2 "></i>
                              <a  class="text-primary show-display" href="{{route('admin.personal-page.comment.hide-show', $ListComment->id)}}">Mở hiển thị</a>
                          </div>
                          <br>
                      @endif

                      @if($ListComment->is_locked == 1 && time() < $ListComment->lock_time)
                          <div class="float-left ml-2 cusor-point mb-1">
                              <i class="icon-setup fa fa-undo mr-2 "></i>
                              <a  class="text-primary un-locked" href="{{route('admin.personal-page.comment.locked-account', $ListComment->user_id)}}">Mở chặn tài khoản</a>
                          </div>
                          <br>
                      @else
                          <div class="float-left ml-2 cusor-point mb-1">
                              <i class="icon-setup fa fa-times mr-2 "></i>
                              <a class="text-danger locked" href="{{route('admin.personal-page.comment.locked-account', $ListComment->user_id)}}">Chặn tài khoản</a>
                          </div>
                          <br>
                      @endif
                      ...
                      @if($ListComment->is_forbidden == 1)
                          <div class="float-left ml-2 cusor-point mb-1">
                              <i class="icon-setup fa fa-undo mr-2 "></i>
                              <a  class="un-forbidden text-primary" href="{{route('admin.personal-page.comment.forbidden-account', $ListComment->user_id)}}">Mở cấm tài khoản</a>
                          </div>
                          <br>
                      @else
                          <div class="float-left ml-2 cusor-point mb-1">
                              <i class="icon-setup fa fa-times mr-2 "></i>
                              <a  class="text-danger forbidden" href="{{route('admin.personal-page.comment.forbidden-account', $ListComment->user_id)}}">Cấm tài khoản</a>
                          </div>
                          <br>
                      @endif
                    @endif
                    ...
                    @if($check_role == 1  ||key_exists(5, $check_role))
                        <div class="float-left ml-2 cusor-point mb-1">
                          <i class="icon-setup fa fa-times mr-2 "></i>
                          <a  class="text-danger delete" href="{{route('admin.personal-page.comment.delete-comment', $ListComment->id)}}">Xóa bình luận</a>
                        </div>
                        <br>
                    @else
                        @if(key_exists(5, $check_role) == false && key_exists(2, $check_role) == false)
       ...
       <div class="text-left d-flex align-items-center">
         <div class="manipulation d-flex mr-4 ">
           ...
           <div class="dropdown-menu">
            @if($check_role == 1  ||key_exists(2, $check_role))
            <a class="dropdown-item wrong-list" type="button" href="{{route('admin.personal-page.comment.wrong-report-list')}}">
           ...
           <a class="dropdown-item un-forbidden-list" type="button" href="{{route('admin.personal-page.comment.un-forbidden-account-list')}}">
             <i class="fa fa-undo bg-primary p-1 mr-2 rounded" style="color: white !important;font-size: 15px;width: 23px"></i>Mở cấm tài khoản
             <input type="hidden" name="action" value="trash">
           </a>
           <a class="dropdown-item forbidden-list" type="button" href="{{route('admin.personal-page.comment.forbidden-account-list')}}">
             <i class="fa fa-times bg-danger p-1 mr-2 rounded" style="color: white !important;font-size: 16px;width: 23px"></i>Cấm tài khoản
             <input type="hidden" name="action" value="trash">
           </a>
           <a class="dropdown-item unlocked-list" type="button" href="{{route('admin.personal-page.comment.unlocked-account-list')}}">
               <i class="fa fa-undo bg-primary p-1 mr-2 rounded" style="color: white !important;font-size: 15px;width: 23px"></i>Mở chặn tài khoản
               <input type="hidden" name="action" value="trash">
           </a>
           <a class="dropdown-item locked-list" type="button" href="{{route('admin.personal-page.comment.locked-account-list')}}">
             <i class="fa fa-times bg-danger p-1 mr-2 rounded" style="color: white !important;font-size: 16px;width: 23px"></i>Chặn tài khoản
             <input type="hidden" name="action" value="trash">
           </a>
           @endif
           @if($check_role == 1  ||key_exists(5, $check_role))
           <a class="dropdown-item moveToTrash" type="button" href="{{route('admin.personal-page.comment.trashlist-comment')}}">
...
<script type="text/javascript">
    $('.block-display').click(function (e) {
        ...
        Swal.fire({
            title: 'Chặn hiển thị',
            text: "Bình luận sẽ được đưa về trạng thái chặn hiển thị",
            icon: 'warning',
    ...
    $('.show-display').click(function (e) {
        ...
        Swal.fire({
            title: 'Hiển thị',
            text: "Bình luận sẽ được đưa về trạng thái hiển thị",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#3085d6',
